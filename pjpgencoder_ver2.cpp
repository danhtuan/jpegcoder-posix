...
void loadImg(char* file);
void printImg(img_data pimg_data);

void *pth_compress(void* pth_data);
...
int main(int argc, char** argv)
{
  ...
  printf("\nLoad image %.6lf second esplaped \n", t2-t1);
  

  //INITIALIZATION
  threadIDs = new pthread_t[num_threads];
  pth_data = new thread_data[num_threads];
  //compute portion of data of each thread
  //pre-compute DCcomp
  int iblk_row = (pimg_data.row + 7)/8;
  int iblk_thread = (iblk_row + num_threads - 1)/num_threads;
  for(i=0;i<num_threads;i++){
    pth_data[i].order = i;
    pth_data[i].b_row = i * iblk_thread * 8;
    int e_row = pth_data[i].b_row + iblk_thread * 8 - 1; 
    if(e_row < pimg_data.row){
      pth_data[i].e_row = e_row;
    }
    else{
      pth_data[i].e_row = pimg_data.row - 1;
    }
    cout<<"\nThread "<<i<<" Row: "<<pth_data[i].b_row<<"-"<<pth_data[i].e_row;
    //pre-compute DCComp
    if(i<1){
      pth_data[i].DCcomp = 0;
    }else{
      int topleft_X = pimg_data.col - 8;
      int topleft_Y = i*iblk_thread*8 - 8;
      //cout<<"\n DC Topleft "<<topleft_Y<<"-"<<topleft_X;
      int f[8][8];
      for(int i1 = 0; i1<8;i1++)
	for(int j1=0;j1<8;j1++)
	  f[i1][j1] = pimg_data.intensity[topleft_Y+i1][topleft_X+j1];
      int F[8][8];
      int QF[8][8];
      DCT(f, F);
      Quantize(F, QF);
      pth_data[i].DCcomp = QF[0][0];
    }        
  }
  gettimeofday(&tim, NULL);
  double t3=tim.tv_sec+(tim.tv_usec/1000000.0);
  printf("\nInit Thread: %.6lf second esplaped \n", t3-t2);

  for(i=0;i<num_threads;i++)
  {
    rc = pthread_create(&threadIDs[i], NULL, pth_compress, (void*) &pth_data[i]);
  }
  ...
  for(i=0;i<num_threads;i++)
  {
    ...
    if (rc) {
      ...
      cout<<"\nMain: completed join with thread "<<i<<" having a status of "<<(long)status;
      gettimeofday(&tim, NULL);
      double tt=tim.tv_sec+(tim.tv_usec/1000000.0);
      printf(" in %.6lf second esplaped \n", tt-t3);
      }
  }
  gettimeofday(&tim, NULL);
  double t4=tim.tv_sec+(tim.tv_usec/1000000.0);
  printf("Thread Compress: %.6lf second esplaped \n", t4-t3);
  ...
  printf("Total time: %.6lf second esplaped \n", t5-t1);
	...
	if((i<maxY)&&(j<maxX))
	  {
	    ...
	    intensity[i][j]=(int)(gray.shade()*cRes)-128;
	  }
	else
	  intensity[i][j] = 0;
      }
  pimg_data.intensity = intensity;
  pimg_data.col = col8;
  pimg_data.row = row8;
...
void *pth_compress(void* pth_data)
{
  ...
  int DCcomp = my_data->DCcomp;
  //cout<<"\nThread "<<(int)my_data->order;
  
  for(i=my_data->b_row;i<=my_data->e_row;i=i+8)
    for(j=0;j<pimg_data.col;j=j+8)
    {	
	for(i1=0;i1<8;i1++)
	  for(j1=0;j1<8;j1++)
	    {
	      f[i1][j1] = pimg_data.intensity[i+i1][j+j1];
	    }	
	//cout<<"\nThread "<<my_data->order<<" Block: "<<i<<"-"<<j<<"\n";
	DCcomp = Compress(f,DCcomp,code);
	strcat(my_data->writecode,code);
    }  
  //cout<<"\nTime in thread "<<(float)(clock()-clk)/CLOCKS_PER_SEC;
}
